...
int gammadof=0, gammadof1=0, gammadof2=0;
getfem::mesh_region &gamma = mesht.region(descr_transp.GAMMA);
for (getfem::mr_visitor i(gamma); !i.finished(); ++i) {
gammadof++;
}
getfem::mesh_region &gamma1 = mesht.region(descr_transp.GAMMA+1);
for (getfem::mr_visitor i(gamma1); !i.finished(); ++i) {
gammadof1++;
}
getfem::mesh_region &gamma2 = mesht.region(descr_transp.GAMMA+2);
for (getfem::mr_visitor i(gamma2); !i.finished(); ++i) {
gammadof2++;
}

	sparse_matrix_type M0a(dof_transp.Ct_Sigma(), dof_transp.Ct_Sigma());gmm::clear(M0a);
	sparse_matrix_type M0b(dof_transp.Ct_Sigma(), dof_transp.Ct_Sigma());gmm::clear(M0b);
	sparse_matrix_type M0c(dof_transp.Ct_Sigma(), dof_transp.Ct_Sigma());gmm::clear(M0c);
	sparse_matrix_type M1a(dof_transp.Ct_Omega(), dof_transp.Ct_Omega());gmm::clear(M1a);
	sparse_matrix_type M1b(dof_transp.Ct_Omega(), dof_transp.Ct_Omega());gmm::clear(M1b);
	sparse_matrix_type M1c(dof_transp.Ct_Omega(), dof_transp.Ct_Omega());gmm::clear(M1c);
	sparse_matrix_type M2a(dof_transp.Ct(), dof_transp.Ct());gmm::clear(M2a);
	sparse_matrix_type M2b(dof_transp.Ct(), dof_transp.Ct());gmm::clear(M2b);
	sparse_matrix_type M2c(dof_transp.Ct(), dof_transp.Ct());gmm::clear(M2c);
	sparse_matrix_type M2_(dof_transp.Ct(), dof_transp.Ct());gmm::clear(M2_);
	sparse_matrix_type M2b_(dof_transp.Ct(), dof_transp.Ct());gmm::clear(M2b_);
	sparse_matrix_type M2c_(dof_transp.Ct(), dof_transp.Ct());gmm::clear(M2c_);

	vector_type F0a(dof_transp.Ct_Sigma(),1);
	vector_type F0b(dof_transp.Ct_Sigma(),1);
	vector_type F0c(dof_transp.Ct_Sigma(),1);
	vector_type F1a(dof_transp.Ct_Omega(),1);
	vector_type F1b(dof_transp.Ct_Omega(),1);
	vector_type F1c(dof_transp.Ct_Omega(),1);
	vector_type F2a(dof_transp.Ct(),1);
	vector_type F2b(dof_transp.Ct(),1);
	vector_type F2c(dof_transp.Ct(),1);
	vector_type F2_(dof_transp.Ct(),1);
	vector_type F2b_(dof_transp.Ct(),1);
	vector_type F2c_(dof_transp.Ct(),1);

	scalar_type check = PARAM.real_value("check", "gamma o omega?");
if (check==1){
	getfem::asm_mass_matrix (M0a, mimt, mf_Ct_Sigma);//, descr_transp.GAMMA); )//
	getfem::asm_mass_matrix (M0b, mimt, mf_Ct_Sigma);//, descr_transp.GAMMA+2);	
	getfem::asm_mass_matrix (M0c, mimt, mf_Ct_Sigma);//, descr_transp.GAMMA+2);	
	getfem::asm_mass_matrix (M1a, mimt, mf_Ct_Omega);//, descr_transp.GAMMA);	
	getfem::asm_mass_matrix (M1b, mimt, mf_Ct_Omega);//, descr_transp.GAMMA+1);
	getfem::asm_mass_matrix (M1c, mimt, mf_Ct_Omega);//, descr_transp.GAMMA+1);	
	getfem::asm_mass_matrix (M2a, mimt, mf_Ct);//, descr_transp.GAMMA);	
	getfem::asm_mass_matrix (M2b, mimt, mf_Ct);//, descr_transp.GAMMA+1);	
	getfem::asm_mass_matrix (M2c, mimt, mf_Ct);//, descr_transp.GAMMA+2);	

	gmm::mult(M0a, F0a, F0a); // gmm::mult(M1, V2, V1);  M1 * V2 --> V1
	gmm::mult(M0b, F0b, F0b);
	gmm::mult(M0c, F0c, F0c);
	gmm::mult(M1a, F1a, F1a);
	gmm::mult(M1b, F1b, F1b);
	gmm::mult(M1c, F1c, F1c);
	gmm::mult(M2a, F2a, F2a);
	gmm::mult(M2b, F2b, F2b);
	gmm::mult(M2c, F2c, F2c);
	
}else if (check==2)
 {	getfem::asm_mass_matrix (M0a, mimt, mf_Ct_Sigma, descr_transp.GAMMA); //
	getfem::asm_mass_matrix (M0b, mimt, mf_Ct_Sigma, descr_transp.GAMMA+1);	
	getfem::asm_mass_matrix (M0c, mimt, mf_Ct_Sigma, descr_transp.GAMMA+2);	
	getfem::asm_mass_matrix (M1a, mimt, mf_Ct_Omega, descr_transp.GAMMA);	
	getfem::asm_mass_matrix (M1b, mimt, mf_Ct_Omega, descr_transp.GAMMA+1);	
	getfem::asm_mass_matrix (M1c, mimt, mf_Ct_Omega, descr_transp.GAMMA+2);	
	getfem::asm_mass_matrix (M2a, mimt, mf_Ct, descr_transp.GAMMA);	
	getfem::asm_mass_matrix (M2b, mimt, mf_Ct, descr_transp.GAMMA+1);	
	getfem::asm_mass_matrix (M2c, mimt, mf_Ct, descr_transp.GAMMA+2);
...
gmm::add(M2a,M2_);
gmm::add(gmm::scaled(M2b,-1.0),M2_);
gmm::add(gmm::scaled(M2c,-1.0),M2_);   //// M2_ becomes ZERO! in the vessel, only the outer boundary contributes remain. 

gmm::add(M2b,M2b_);
gmm::add(gmm::scaled(M2a,-0.5),M2b_);
gmm::add(M2c,M2c_);
gmm::add(gmm::scaled(M2a,-0.5),M2c_); 
	...
	gmm::mult(M2b, F2b, F2b);
	gmm::mult(M2c, F2c, F2c);
	gmm::mult(M2_, F2_, F2_);
	gmm::mult(M2b_, F2b_, F2b_);
	gmm::mult(M2c_, F2c_, F2c_);


} else if (check==3){
	vector_type F0(dof_transp.Ct_Sigma(),1);
	vector_type F1(dof_transp.Ct_Omega(),1);
	vector_type F2(dof_transp.Ct(),1);
gmm::clear(F0a);
gmm::clear(F0b);
gmm::clear(F0c);
gmm::clear(F1a);
...
getfem::asm_source_term (F0a, mimt, mf_Ct_Sigma, mf_Ct_Sigma, F0, descr_transp.GAMMA);
getfem::asm_source_term (F0b, mimt, mf_Ct_Sigma, mf_Ct_Sigma, F0, descr_transp.GAMMA+1);
getfem::asm_source_term (F0c, mimt, mf_Ct_Sigma, mf_Ct_Sigma, F0, descr_transp.GAMMA+2);
getfem::asm_source_term (F1a, mimt, mf_Ct_Omega, mf_Ct_Omega, F1, descr_transp.GAMMA);
getfem::asm_source_term (F1b, mimt, mf_Ct_Omega, mf_Ct_Omega, F1, descr_transp.GAMMA+1);
getfem::asm_source_term (F1c, mimt, mf_Ct_Omega, mf_Ct_Omega, F1, descr_transp.GAMMA+2);
getfem::asm_source_term (F2a, mimt, mf_Ct, mf_Ct, F2, descr_transp.GAMMA);
getfem::asm_source_term (F2b, mimt, mf_Ct, mf_Ct, F2, descr_transp.GAMMA+1);
getfem::asm_source_term (F2c, mimt, mf_Ct, mf_Ct, F2, descr_transp.GAMMA+2);

}
else if (check==4){
	vector_type F0(dof_transp.Ct(),1);
	vector_type F1(dof_transp.Ct(),1);
	vector_type F2(dof_transp.Ct(),1);
gmm::clear(F0a);
gmm::clear(F0b);
gmm::clear(F0c);
...
getfem::asm_source_term (F0a, mimt, mf_Ct_Sigma, mf_Ct, F0, descr_transp.GAMMA);
getfem::asm_source_term (F0b, mimt, mf_Ct_Sigma, mf_Ct, F0, descr_transp.GAMMA+1);
getfem::asm_source_term (F0c, mimt, mf_Ct_Sigma, mf_Ct, F0, descr_transp.GAMMA+2);
getfem::asm_source_term (F1a, mimt, mf_Ct_Omega, mf_Ct, F1, descr_transp.GAMMA);
getfem::asm_source_term (F1b, mimt, mf_Ct_Omega, mf_Ct, F1, descr_transp.GAMMA+1);
getfem::asm_source_term (F1c, mimt, mf_Ct_Omega, mf_Ct, F1, descr_transp.GAMMA+2);
getfem::asm_source_term (F2a, mimt, mf_Ct, mf_Ct, F2, descr_transp.GAMMA);
getfem::asm_source_term (F2b, mimt, mf_Ct, mf_Ct, F2, descr_transp.GAMMA+1);
getfem::asm_source_term (F2c, mimt, mf_Ct, mf_Ct, F2, descr_transp.GAMMA+2);
	...
	vtk_export exp_F0a(descr_transp.OUTPUT+"F0a.vtk");
	exp_F0a.exporting(mf_Ct_Sigma);
	exp_F0a.write_mesh();
	exp_F0a.write_point_data(mf_Ct_Sigma, F0a, "F0a");

	vtk_export exp_F0b(descr_transp.OUTPUT+"F0b.vtk");
	exp_F0b.exporting(mf_Ct_Sigma);
	exp_F0b.write_mesh();
	exp_F0b.write_point_data(mf_Ct_Sigma, F0b, "F0b");

	vtk_export exp_F0c(descr_transp.OUTPUT+"F0c.vtk");
	exp_F0c.exporting(mf_Ct_Sigma);
	exp_F0c.write_mesh();
	exp_F0c.write_point_data(mf_Ct_Sigma, F0c, "F0c");


	vtk_export exp_F1a(descr_transp.OUTPUT+"F1a.vtk");
	exp_F1a.exporting(mf_Ct_Omega);
	exp_F1a.write_mesh();
	exp_F1a.write_point_data(mf_Ct_Omega, F1a, "F1a");

	vtk_export exp_F1b(descr_transp.OUTPUT+"F1b.vtk");
	exp_F1b.exporting(mf_Ct_Omega);
	exp_F1b.write_mesh();
	exp_F1b.write_point_data(mf_Ct_Omega, F1b, "F1b");

	vtk_export exp_F1c(descr_transp.OUTPUT+"F1c.vtk");
	exp_F1c.exporting(mf_Ct_Omega);
	exp_F1c.write_mesh();
	exp_F1c.write_point_data(mf_Ct_Omega, F1c, "F1c");

	vtk_export exp_F1b_(descr_transp.OUTPUT+"F1b_.vtk");
	exp_F1b_.exporting(mf_Ct);
	exp_F1b_.write_mesh();
	exp_F1b_.write_point_data(mf_Ct_Omega, F1b, "F1b_");


	vtk_export exp_F2a(descr_transp.OUTPUT+"F2a.vtk");
	exp_F2a.exporting(mf_Ct);
	exp_F2a.write_mesh();
	exp_F2a.write_point_data(mf_Ct, F2a, "F2a");

	vtk_export exp_F2b(descr_transp.OUTPUT+"F2b.vtk");
	exp_F2b.exporting(mf_Ct);
	exp_F2b.write_mesh();
	exp_F2b.write_point_data(mf_Ct, F2b, "F2b");

	vtk_export exp_F2c(descr_transp.OUTPUT+"F2c.vtk");
	exp_F2c.exporting(mf_Ct);
	exp_F2c.write_mesh();
	exp_F2c.write_point_data(mf_Ct, F2c, "F2c");

	vtk_export exp_F2_(descr_transp.OUTPUT+"F2_.vtk");
	exp_F2_.exporting(mf_Ct);
	exp_F2_.write_mesh();
	exp_F2_.write_point_data(mf_Ct, F2_, "F2_");

	vtk_export exp_F2b_(descr_transp.OUTPUT+"F2b_.vtk");
	exp_F2b_.exporting(mf_Ct);
	exp_F2b_.write_mesh();
	exp_F2b_.write_point_data(mf_Ct, F2b_, "F2b_");

	vtk_export exp_F2c_(descr_transp.OUTPUT+"F2c_.vtk");
	exp_F2c_.exporting(mf_Ct);
	exp_F2c_.write_mesh();
	exp_F2c_.write_point_data(mf_Ct, F2c_, "F2c_");

cout <<"-------------------------------------------------------------------"<<endl;
cout <<"-------------------------------------------------------------------"<<endl;
cout <<"-------------------------------------------------------------------"<<endl;

cout <<"dof sigma: "<<dof_transp.Ct_Sigma()<<endl;
cout <<"dof omega plus (holed): "<<dof_transp.Ct_Omega()<<endl;
cout <<"dof omega (whole): "<<dof_transp.Ct()<<endl;
cout <<"dof gamma (sigma+omega): "<<gammadof<<endl;
cout <<"dof gamma1(omega): "<<gammadof1<<endl;
cout <<"dof gamma2(sigma): "<<gammadof2<<endl;
cout <<"-------------------------------------------------------------------"<<endl;
cout <<"-------------------------------------------------------------------"<<endl;
cout <<"-------------------------------------------------------------------"<<endl;
...
if(ASSUMPTION==A0){
	gmm::resize(AM_transp, dof_v, dof_v);	gmm::clear(AM_transp);
	gmm::resize(UM_transp, dof_v);		gmm::clear(UM_transp);
	gmm::resize(FM_transp, dof_v);	 	gmm::clear(FM_transp);
	}
else 
if(ASSUMPTION==A1 || ASSUMPTION==A2){
	gmm::resize(AM_transp, dof_t, dof_t);	gmm::clear(AM_transp);
	gmm::resize(UM_transp, dof_t);		gmm::clear(UM_transp);
	gmm::resize(FM_transp, dof_t);	 	gmm::clear(FM_transp);
	}
...
if(ASSUMPTION==A3){
	...
	// Assemble At, stiffness matrix for laplacian in tissue
	if (ASSUMPTION==A0){
		// do nothing! A0 is only on vessel
	}
	else if(ASSUMPTION==A1){
		getfem::asm_stiffness_matrix_for_homogeneous_laplacian(At,mimt,mf_Ct_Omega); 	}
	else{
	...
	if(ASSUMPTION==A0){
		...
		//do nothing! A1 and A2 are only on tissue
	}
...
gmm::scale(Btt, 0.5);
	...
	else if(ASSUMPTION ==A1 || ASSUMPTION == A2){
	// Copying Btt
		gmm::add(Btt,			 
			gmm::sub_matrix(AM_transp, 
					gmm::sub_interval(0, dof_t), 
	...
	#ifdef M3D1D_VERBOSE_
	cout << " Prepare AM_temp and FM_temp ..." << endl;
	#endif

if(ASSUMPTION==A0){
	gmm::resize(AM_temp, dof_v,
			     dof_v);	gmm::clear(AM_temp);
	gmm::resize(FM_temp, dof_v); 	gmm::clear(FM_temp);
	gmm::copy(AM_transp,AM_temp);
	gmm::copy(FM_transp,FM_temp);
...
if(ASSUMPTION==A1 || ASSUMPTION==A2){	
	gmm::resize(AM_temp, dof_t,
			     dof_t);	gmm::clear(AM_temp);
	gmm::resize(FM_temp, dof_t); 	gmm::clear(FM_temp);
	gmm::copy(AM_transp,AM_temp);
	gmm::copy(FM_transp,FM_temp);
	}
...
if(ASSUMPTION==A3){	
	...
	for (size_type bc=0; bc < BCt_transp.size(); ++bc) {
	if(ASSUMPTION==A0){
		//do nothing: dirichlet on partial Omega doesn't affect Sigma!
	}else if(ASSUMPTION==A1){
	getfem::assembling_Dirichlet_condition(AM_temp, FM_temp, mf_Ct_Omega, BCt_transp[bc].rg, Dirichlet_null);
	}else{
	getfem::assembling_Dirichlet_condition(AM_temp, FM_temp, mf_Ct,       BCt_transp[bc].rg, Dirichlet_null);}
	...
	//! Assemble problem Z1
	void transport3d1d::assembly_dual_model_old(const size_type ASSUMPTION){
...
//VOGLIO USARE SAMG! Trova un modo per generalizzare solver_transp().SAMG, perchè al momento vale solo per matrici con dof_transp.Ct() x dof_transp.Cv() gradi di libertà --> problemi per convergence e per model, che hanno diversi gradi di libertà.
// qui riportiamo solo SUPERLU e SAMG
...
// solver SAMG: tutto questo dovrà essere sostituito da solver_transp()
	...
	switch(VERSION){
		...
		switch(ASSUMPTION){
			...
			case A0: 
			{	solver(dof_v,0,0,0);  // only vessel (3d vessel!)
				break;
			}
			case A1: 
			{	solver(dof_t,0,0,0); // only tissue (holed omega!)
				break;
			}
			case A2: 
			{	solver(dof_t,0,0,0); // only tissue (whole omega!)
				break;
			}
			case A3: 
			{	solver(dof_t,dof_v,0,0); //both tissue and vessel
				break;
			}
		 }
				break;} 
